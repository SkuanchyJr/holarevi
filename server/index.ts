...
import { initActivationEmailScheduler } from "./jobs/activationEmailScheduler";
import { initTrialExpirationScheduler } from "./jobs/trialExpirationScheduler";
import { initNfcUpsellScheduler } from "./jobs/nfcUpsellScheduler";
import { initWinbackScheduler } from "./jobs/winbackScheduler";
...
import { addEmailSequenceLogsTable } from "./migrations/add-email-sequence-logs-table";
  ...
  try {
    ...
    if (replitDomains) {
      ...
      const { webhook, uuid } = await stripeSync.findOrCreateManagedWebhook(
        ...
        {
          enabled_events: ["*"],
          description: "Managed webhook for HolaRevi",
        },
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!Buffer.isBuffer(req.body)) {
        log("Webhook body is not a Buffer", "stripe");
...
(async () => {
  ...
  await addEmailSequenceLogsTable();
  ...
  initActivationEmailScheduler(app);
  initTrialExpirationScheduler(app);
  initNfcUpsellScheduler(app);
  initWinbackScheduler(app);
