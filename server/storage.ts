...
export class DatabaseStorage implements IStorage {
  ...
  async getAdvancedAnalytics(userId: string): Promise<AdvancedAnalytics> {
    ...
    const emptyResult: AdvancedAnalytics = {
      ...
      comparison: { reviewsDelta: 0, reviewsDeltaPct: null, ratingDelta: 0, responseRateDelta: 0, replyTimeDelta: null },
      performanceScore: 0,
      sentimentOverTime: [],
    };
    ...
    const currentPeriod = allReviews.filter(r => {
      const d = r.createdAt ? new Date(r.createdAt) : null;
      return d !== null && d >= thirtyDaysAgo;
    });
    const prevPeriod = allReviews.filter(r => {
      const d = r.createdAt ? new Date(r.createdAt) : null;
      return d !== null && d >= sixtyDaysAgo && d < thirtyDaysAgo;
    });
    const curRevCount  = currentPeriod.length;
    const prevRevCount = prevPeriod.length;
    const reviewsDelta    = curRevCount - prevRevCount;
    const reviewsDeltaPct = prevRevCount === 0 ? null
      : Math.round(((curRevCount - prevRevCount) / prevRevCount) * 1000) / 10;
    const curRating  = curRevCount > 0
      ? Math.round((currentPeriod.reduce((s, r) => s + r.rating, 0) / curRevCount) * 10) / 10 : 0;
    const prevRating = prevRevCount > 0
      ? Math.round((prevPeriod.reduce((s, r) => s + r.rating, 0) / prevRevCount) * 10) / 10 : 0;
    const ratingDelta = Math.round((curRating - prevRating) * 10) / 10;
    const curPosted    = currentPeriod.filter(r => r.replyStatus === "posted").length;
    const prevPosted   = prevPeriod.filter(r => r.replyStatus === "posted").length;
    const curRespRate  = curRevCount  > 0 ? Math.round((curPosted  / curRevCount)  * 100) : 0;
    const prevRespRate = prevRevCount > 0 ? Math.round((prevPosted / prevRevCount) * 100) : 0;
    const responseRateDelta = curRespRate - prevRespRate;
    function avgReplyTimeFor(subset: typeof allReviews): number | null {
      const replied = subset.filter(r => r.repliedAt && r.reviewedAt);
      if (!replied.length) return null;
      const ms = replied.reduce((s, r) =>
        s + Math.max(0, new Date(r.repliedAt!).getTime() - new Date(r.reviewedAt!).getTime()), 0);
      return Math.round((ms / replied.length / 3_600_000) * 10) / 10;
    }
    const curReplyTime  = avgReplyTimeFor(currentPeriod);
    const prevReplyTime = avgReplyTimeFor(prevPeriod);
    const replyTimeDelta = (curReplyTime !== null && prevReplyTime !== null)
      ? Math.round((curReplyTime - prevReplyTime) * 10) / 10 : null;
    const comparison = { reviewsDelta, reviewsDeltaPct, ratingDelta, responseRateDelta, replyTimeDelta };
    ...
    const ratingNorm = totalReviews > 0 ? Math.round(((averageRating - 1) / 4) * 40) : 0;
    const respNorm   = Math.round((responseRate / 100) * 30);
    const posRatio   = totalReviews > 0 ? sentimentBreakdown.positive / totalReviews : 0;
    const sentNorm   = Math.round(posRatio * 20);
    const speedNorm  = averageReplyTimeHours === null ? 0
      : averageReplyTimeHours < 2 ? 10
      : averageReplyTimeHours < 24 ? 5
      : 0;
    const performanceScore = ratingNorm + respNorm + sentNorm + speedNorm;
    ...
    // Monthly trends (last 12 months) + sentiment over time
    const monthlyTrendsMap = new Map<string, { reviews: number; replies: number; ratingSum: number }>();
    const sentimentMonthMap = new Map<string, { positive: number; neutral: number; negative: number }>();
    allReviews.forEach((review) => {
      ...
      const se = sentimentMonthMap.get(month) || { positive: 0, neutral: 0, negative: 0 };
      if (review.sentiment === "positive") se.positive++;
      else if (review.sentiment === "negative") se.negative++;
      else se.neutral++;
      sentimentMonthMap.set(month, se);
    });
    const sentimentOverTime = Array.from(sentimentMonthMap.entries())
