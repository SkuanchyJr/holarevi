...
import { db } from "../db";
import { reviews, restaurants, users, nfcOrders } from "@shared/schema";
...
export async function computeWeeklyMetrics(
  ...
  try {
    ...
    // NFC order check (by email, no FK link to users)
    const userEmail = user.email || "";
    let hasNfcOrder = false;
    if (userEmail) {
      const [nfcOrder] = await db
        .select({ id: nfcOrders.id })
        .from(nfcOrders)
        .where(and(eq(nfcOrders.email, userEmail), eq(nfcOrders.status, "paid")))
        .limit(1);
      hasNfcOrder = !!nfcOrder;
    }
