...
// ─── Public helpers for welcome email (triggered from auth.ts) ───────────────
...
export async function sendWelcomeEmail(
  ...
  const template = buildWelcomeEmail(firstName, language);
  return sendEmail(email, template.subject, template.html, "WelcomeEmail");
}

export async function logWelcomeEmail(userId: string): Promise<void> {
  await logResult(userId, 0, "success");
}
